/* diffuse1d_omp.c: OpenMP 1d Diffusion simulation with textual output.
...
static void quit() {
  printf("Usage: diffuse1d_omp.exec NX K NSTEPS WSTEP [FILENAME]      \n\
...
static void setup(int argc, char * argv[]) {
  ...
  if (!(nx>=2 && 0<k && k<.5 && nstep>=1 && wstep>=0 && wstep<=nstep))
    quit();
  printf("Starting diffuse1d_omp: nx=%d k=%lf nstep=%d wstep=%d, \
max_threads=%d\n", nx, k, nstep, wstep, omp_get_max_threads());
  fflush(stdout);
...
static void teardown() {
  ...
  printf("Finished diffuse1d_omp: time = %lf\n", omp_get_wtime() - start_time);
}
