/* In this CUDA version of diffuse1d, each thread operates on one
   element, but all elements in the block are first loaded into shared
   memory. */
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
...
update(int n, double k, double * t_old, double * t_new) {
  __shared__ double tmp[threadsPerBlock+2];
  const int nl = blockDim.x; // number of threads per block
  const int lidx = threadIdx.x + 1; // index of tmp I am updating
  const int gidx = blockIdx.x*nl + lidx; // corresponding index of t_old and t_new

  // load the one cell for this thread into shared memory...
  if (gidx < n) tmp[lidx] = t_old[gidx];
  // load the ghost/boundary cells for this block into shared memory...
  if (lidx == 1 && gidx < n) tmp[0] = t_old[gidx-1];
  if (lidx == nl && gidx < n-1) tmp[nl+1] = t_old[gidx+1];

  __syncthreads();  // barrier for this block

  if (0 < gidx && gidx < n-1)
    t_new[gidx] = tmp[lidx] + k*(tmp[lidx+1] + tmp[lidx-1] - 2*tmp[lidx]);
}
