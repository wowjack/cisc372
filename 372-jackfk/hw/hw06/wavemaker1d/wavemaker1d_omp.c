  ...
  #pragma omp parallel
{
  ...
  printf("wavemaker1d: nx=%d ny=%d width=%d k=%.3lf nstep=%d wstep=%d nthreads=%d\n",
	 nx, ny, width, k, nstep, wstep, numthreads);
...
//might as well parallelize this part. Doubt it will help much but also couldn't hurt much
#pragma omp parallel for default(shared) schedule(guided, 1)
  for (int i = 0; i < nx; i++)
    u_curr[i] = u_prev[i] =
      (i == 0 || i >= width ? 0.0 :
...
void update() {
//We only write the u_next array and only at index i, so there is no data race
#pragma omp parallel for default(shared) schedule(guided, 1)
...
int main(int argc, char *argv[]) {
  ...
  //We cannot parallelize this loop because each iteration of update requires
  //data calculated by the previous iteration
