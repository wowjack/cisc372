...
static void init(int numThreads, char* filename) {
...
void* threadFunc(void* arg){
  ...
  for(int i=threadNum; i<nbodies; i+=nThreads){
...
static inline void update() {
  ...
  //I dont need a barrier here, just need to wait for every thread to return.
  //So I thought a join loop would work just fine.
  for(int i=0; i<nThreads; i++){
