...
#pragma omp parallel for shared(a) default(none)
  ...
  assert(err == cudaSuccess);
  max<<<nblocks, threadsPerBlock>>>(N/2, dev_a, dev_partial_maxs);

  
  T local_max = 0;
  #pragma omp parallel for reduction(max : local_max) default(none)
  for(int i=N/2; i<N; i++){
    if(sin(i) > local_max) local_max = sin(i);
  }
  ...
  T result = local_max;
#pragma omp parallel for shared(partial_maxs, result) default(none)
